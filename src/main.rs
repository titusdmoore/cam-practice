use nokhwa::{
    ...
    query,
    utils::{ApiBackend, RequestedFormat, RequestedFormatType, FrameFormat},
    Camera,
};
...
fn main() {
    ...
    let raw_image = <RgbFormat as FormatDecoder>::write_output(FrameFormat::YUYV, buffer.resolution(), buffer.buffer()).unwrap();
    image::save_buffer("img.png", &raw_image.to_owned(), buffer.resolution().width(), buffer.resolution().height(), image::ColorType::Rgb8).unwrap();
